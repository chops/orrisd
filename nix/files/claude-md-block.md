...
Codex CLI in the adjacent pane. Both panes share a per-project inbox at
`$AI_PAIR_INBOX`.
...
If you don't see `$AI_PAIR_INBOX` set, you are NOT inside an ai-pair
session — fall back to normal Claude Code behavior and do not attempt
peer-protocol I/O.
