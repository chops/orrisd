...
D01 clarification, 2026-09-20: the source-adoption candidate also advertises
sessions_read in ordinary v2 ping. The live-store condition above still governs
delivery_reconcile; "only" states that condition, not a singleton capability list.
sessions_read is a census-read capability, not durable-attachment negotiation.
Producer qualification and coordinated pairing remain pending in
docs/contracts/ipc-v2.org; this note does not release a main or live rollout.
...
** Addendum 2026-09-25: last raw capture not retained in pane data (NS-17 F-1)
:PROPERTIES:
:CUSTOM_ID: addendum-20260925-last-raw-capture-not-retained
:END:
Dated 2026-09-25. The pane state machine's data no longer holds the last
raw capture: the struct field that kept it is removed, and a successful
poll stores only the stripped-content hash. The last raw capture is
therefore not emitted through the ~:state~ of the pane's gen_statem
terminate report. This addendum claims nothing more. It does not claim
that OTP status output, other terminate-report fields, logs, or process
memory are free of pane or prompt bytes: a capture being processed, legacy
unreceipted queue entries, classifier exception arguments and
whole-process dumps remain outside it, and the non-claim above stands.
