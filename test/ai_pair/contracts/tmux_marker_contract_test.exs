defmodule AiPair.Contracts.TmuxMarkerContractTest do
  ...
  The byte-level reader (`AiPair.PaneRestore.Marker.read/2`: trim, ordered decode, duplicate
  key rejection, exact shape) is the R04 S5 slice and is not on main yet, so this test does
  not exercise it. It pins the expected classification table and checks, for every fixture
  that decodes to a unique-key document, that the pure decision layer already on main,
  `AiPair.PaneRestore.Admission.admit/4`, reaches the same finding when handed the decoded
  document as the marker source. Duplicate-key and non-JSON fixtures are asserted at the
  decoder boundary only; they are the cases S5 must refuse before Admission ever sees them.
  """
  ...
  # marker.ex:209 trims the trailing newline show-options appends; the stored value
  # itself carries none.
